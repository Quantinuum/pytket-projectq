...
`pytket-projectq` is an extension to `pytket` that allows `pytket` circuits to be
executed on ProjectQ simulators, as well as conversion to the ProjectQ
representation.
...
`pytket-projectq` is compatible with Python versions 3.10 to 3.13 on Linux, MacOS
